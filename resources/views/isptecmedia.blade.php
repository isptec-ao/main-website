            ...
            <!-- <img src="{!! $content->getFirstMediaUrl('featured_image') !!}" alt="" class="img-thumbnail"> -->
                ...
                <div class="color-box bg-overlay py-4 rounded">
                    ...
                    <img src="{!! $content->getFirstMediaUrl('featured_image') !!}" alt="" class="img-thumbnail" width="20%">
                        ...
                        <p>{!! $content->summary !!}</p>
                        <p><small class="text-muted"> <i class="far fa-calendar"></i> {!! $content->published_at->locale(session()->get('locale'))->diffForHumans() !!}</small></p>
                        <a href="{!! route('aci.showisptecmedia', [$content->slug . '?lang=' . (session()->get('locale') ?? 'pt') . '&id=' . $content->id]) !!}" class="btn btn-outline-brand btn-rounded">@lang('translation.news_read_more')</a>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="col-md-4">
            <div class="card shadow-none">
                ...
                <div class="position-relative">
                    ...
                    <br>
                    <br>
                    <h5><a href="{!! route('aci.showisptecmedia', [$post->slug . '?lang=' . (session()->get('locale') ?? 'pt') . '&id=' . $post->id]) !!}" class="text-dark">{!! $post->title !!}</a></h5>
                ...
                <div class="">
                    ...
                    <div>
                        <a href="{!! route('aci.showisptecmedia', [$post->slug . '?lang=' . (session()->get('locale') ?? 'pt') . '&id=' . $post->id]) !!}" class="text-primary btn btn-sm btn-primary btn-rounded text-light">
                        @lang('translation.news_read_more') <i class="mdi mdi-arrow-right"></i>
                            <i class="mdi mdi-calendar"></i> {!! $post->published_at->format('d/m/Y') !!}
                        </a>
                    </div>
                </div>
            </div>
