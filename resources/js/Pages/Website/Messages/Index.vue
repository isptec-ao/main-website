        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.messages.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.messages.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.messages.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.messages.getlang', {message: id, lang: lang}));
          },
          reset() {
        ...
        <div class="page-title-box d-flex align-items-center justify-content-between">
          ...
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                ...
                <inertia-link class="btn btn-secondary" :href="route('canvas.messages.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="message in $page.props.messages.data" :key="message.id">
                    <td>{{ message.description }}</td>
                    <td>
                      <span v-if="message.gender==='M'">
                          <i class="bx bx-male text-primary"></i> M
                      </span>

                      <span v-if="message.gender==='F'">
                          <i class="bx bx-female text-pink"></i> F
                      </span>

                      <span v-if="message.gender==='A'">
                          <i class="bx bx-male text-primary"></i><i class="bx bx-female text-pink"></i> MF
                      </span>
                    </td>
                   
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.messages.edit', message.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(message.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(message.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(message.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.messages.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.messages.links" :lastpage="$page.props.messages.last_page" :firstpageurl="$page.props.messages.first_page_url" :lastpageurl="$page.props.messages.last_page_url" :nextpageurl="$page.props.messages.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
