<script>
    ...
    export default {
        ...
        layout: (h, dashboard) => h(BaseCanvasLayout, {
            props: {
              newslettersubscriptions: Object,
              filters: Object,
            }
        }, [dashboard]),
        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.newslettersubscriptions.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.newslettersubscriptions.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.newslettersubscriptions.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.newslettersubscriptions.getlang', {newslettersubscription: id, lang: lang}));
          },
          reset() {
        ...
        <div class="page-title-box d-flex align-items-center justify-content-between">
          ...
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                ...
                <inertia-link class="btn btn-secondary" :href="route('canvas.newslettersubscriptions.create')" method="get">
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            ...
            <!-- All Newsletter - Inscrições Table -->
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="newslettersubscription in $page.props.newslettersubscriptions.data" :key="newslettersubscription.id">
                    <td>{{ newslettersubscription.fullname }}</td>
                    <td>{{ newslettersubscription.email }}</td>
                    <td>{{ newslettersubscription.category.name }}</td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.newslettersubscriptions.edit', newslettersubscription.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(newslettersubscription.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(newslettersubscription.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(newslettersubscription.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.newslettersubscriptions.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.newslettersubscriptions.links" :lastpage="$page.props.newslettersubscriptions.last_page" :firstpageurl="$page.props.newslettersubscriptions.first_page_url" :lastpageurl="$page.props.newslettersubscriptions.last_page_url" :nextpageurl="$page.props.newslettersubscriptions.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
