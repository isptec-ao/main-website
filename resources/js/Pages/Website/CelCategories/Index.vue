        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.celcategories.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.celcategories.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.celcategories.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.celcategories.getlang', {celcategory: id, lang: lang}));
          },
          reset() {
    ...
    <div class="row">
      <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
          <h4 class="mb-0 font-size-18">Clubes</h4>
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                <b-button v-b-modal.action-info variant="secondary">
                    <i class="bx bx-info-circle"></i>
                </b-button>
                <inertia-link class="btn btn-secondary" :href="route('canvas.celcategories.create')" method="get">
              ...
              <table class="table table-bordered mb-0">
                <thead>
                  <tr>
                    ...
                    <th>Descrição</th>
                    <th>Responsável</th>
                    <th>Responsável Externo</th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="celcategory in $page.props.celcategories.data" :key="celcategory.id">
                    <td>{{ celcategory.name }}</td>
                    <td>{{ celcategory.description }}</td>
                    <td>{{ celcategory.employee.full_name }}</td>
                    <td>{{ celcategory.responsible_name }}</td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.celcategories.edit', celcategory.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(celcategory.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(celcategory.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(celcategory.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.celcategories.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.celcategories.links" :lastpage="$page.props.celcategories.last_page" :firstpageurl="$page.props.celcategories.first_page_url" :lastpageurl="$page.props.celcategories.last_page_url" :nextpageurl="$page.props.celcategories.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
