<script>
    ...
    export default {
        ...
        layout: (h, dashboard) => h(BaseCanvasLayout, {
            props: {
              clubmemberships: Object,
              filters: Object,
            }
        }, [dashboard]),

        metaInfo: {
            title: 'Inscrições - Clubes'
        },
        components: {
          Pagination,
          SearchFilter
        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.clubmemberships.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.clubmemberships.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.clubmemberships.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.clubmemberships.getlang', {clubmembership: id, lang: lang}));
          },
          reset() {
    ...
    <div class="row">
      <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
          <h4 class="mb-0 font-size-18">Inscrições - Clubes</h4>
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                <b-button v-b-modal.action-info variant="secondary">
                    <i class="bx bx-info-circle"></i>
                </b-button>
                <inertia-link class="btn btn-secondary" :href="route('canvas.clubmemberships.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="clubmembership in $page.props.clubmemberships.data" :key="clubmembership.id">
                    <td>{{ clubmembership.name }}</td>
                    <td>{{ clubmembership.surname }}</td>
                    <td>{{ clubmembership.email }}</td>
                    <td>{{ clubmembership.member_type }}</td>
                    <td>{{ clubmembership.category.name }}</td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.clubmemberships.edit', clubmembership.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(clubmembership.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(clubmembership.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(clubmembership.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.clubmemberships.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.clubmemberships.links" :lastpage="$page.props.clubmemberships.last_page" :firstpageurl="$page.props.clubmemberships.first_page_url" :lastpageurl="$page.props.clubmemberships.last_page_url" :nextpageurl="$page.props.clubmemberships.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
