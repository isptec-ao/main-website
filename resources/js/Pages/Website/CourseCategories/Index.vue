        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.coursecategories.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.coursecategories.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.coursecategories.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.coursecategories.getlang', {coursecategory: id, lang: lang}));
          },
          reset() {
    ...
    <div class="row">
      <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
          <h4 class="mb-0 font-size-18">Cursos - Categorias</h4>
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                <b-button v-b-modal.action-info variant="secondary">
                    <i class="bx bx-info-circle"></i>
                </b-button>
                <inertia-link class="btn btn-secondary" :href="route('canvas.coursecategories.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="academiccategory in $page.props.coursecategories.data" :key="academiccategory.id">
                    <td>{{ academiccategory.name }}</td>
                    <td v-html="academiccategory.description"></td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.coursecategories.edit', academiccategory.id)" method="get">
                  ...
                  <tr v-if="$page.props.coursecategories.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.coursecategories.links" :lastpage="$page.props.coursecategories.last_page" :firstpageurl="$page.props.coursecategories.first_page_url" :lastpageurl="$page.props.coursecategories.last_page_url" :nextpageurl="$page.props.coursecategories.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
