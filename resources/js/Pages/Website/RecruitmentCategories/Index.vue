        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.recruitmentcategories.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.recruitmentcategories.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.recruitmentcategories.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.recruitmentcategories.getlang', {recruitmentcategory: id, lang: lang}));
          },
          reset() {
        ...
        <div class="page-title-box d-flex align-items-center justify-content-between">
          ...
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                ...
                <inertia-link class="btn btn-secondary" :href="route('canvas.recruitmentcategories.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="recruitmentcategory in $page.props.recruitmentcategories.data" :key="recruitmentcategory.id">
                    <td>{{ recruitmentcategory.name }}</td>
                    <td v-html="recruitmentcategory.description"></td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.recruitmentcategories.edit', recruitmentcategory.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(recruitmentcategory.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(recruitmentcategory.id)">
                  ...
                  <tr v-if="$page.props.recruitmentcategories.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.recruitmentcategories.links" :lastpage="$page.props.recruitmentcategories.last_page" :firstpageurl="$page.props.recruitmentcategories.first_page_url" :lastpageurl="$page.props.recruitmentcategories.last_page_url" :nextpageurl="$page.props.recruitmentcategories.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
