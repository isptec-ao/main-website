        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.courseplans.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.courseplans.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.courseplans.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.courseplans.getlang', {courseplan: id, lang: lang}));
          },
          reset() {
        ...
        <div class="page-title-box d-flex align-items-center justify-content-between">
          ...
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                ...
                <inertia-link class="btn btn-secondary" :href="route('canvas.courseplans.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="courseplan in $page.props.courseplans.data" :key="courseplan.id">
                    <td>{{ courseplan.semester.name }}</td>
                    <td>{{ courseplan.course.name }}</td>
                    <td>{{ courseplan.subject.name }}</td>
                    <td>{{ courseplan.theoretical }}</td>
                    <td>{{ courseplan.practical }}</td>
                    <td>{{ courseplan.theoretical_practical }}</td>
                    <td>{{ courseplan.weekly_hours }}</td>
                    <td>{{ courseplan.semester_hours }}</td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.courseplans.edit', courseplan.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(courseplan.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(courseplan.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(courseplan.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.courseplans.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="9">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.courseplans.links" :lastpage="$page.props.courseplans.last_page" :firstpageurl="$page.props.courseplans.first_page_url" :lastpageurl="$page.props.courseplans.last_page_url" :nextpageurl="$page.props.courseplans.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
