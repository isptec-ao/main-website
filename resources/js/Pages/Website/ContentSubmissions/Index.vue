        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.contentsubmissions.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.contentsubmissions.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.contentsubmissions.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.contentsubmissions.getlang', {contentsubmission: id, lang: lang}));
          },
          reset() {
        ...
        <div class="page-title-box d-flex align-items-center justify-content-between">
          ...
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                ...
                <inertia-link class="btn btn-secondary" :href="route('canvas.contentsubmissions.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="contentsubmission in $page.props.contentsubmissions.data" :key="contentsubmission.id">
                    <td>{{ contentsubmission.title }}</td>
                    <td>
                      <span v-if="contentsubmission.category==='N'">
                          Notícia
                      </span>

                      <span v-if="contentsubmission.category==='E'">
                          Evento
                      </span>

                      <span v-if="contentsubmission.category==='O'">
                          Outro
                      </span>

                    </td>
                    <td>{{ contentsubmission.name }}</td>
                    <td>{{ contentsubmission.email }}</td>
                    <td>{{ contentsubmission.contact }}</td>
                    <td>{{ contentsubmission.created_at }}</td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.contentsubmissions.edit', contentsubmission.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(contentsubmission.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(contentsubmission.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(contentsubmission.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.contentsubmissions.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.contentsubmissions.links" :lastpage="$page.props.contentsubmissions.last_page" :firstpageurl="$page.props.contentsubmissions.first_page_url" :lastpageurl="$page.props.contentsubmissions.last_page_url" :nextpageurl="$page.props.contentsubmissions.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
