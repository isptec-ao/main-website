<script>
    ...
    export default {
        ...
        layout: (h, dashboard) => h(BaseCanvasLayout, {
            props: {
              academiccategories: Object,
              filters: Object,
            }
        }, [dashboard]),

        metaInfo: {
            title: 'Graus Académicos'
        },
        components: {
          Pagination,
          SearchFilter
        ...
        watch: {
          form: {
            handler: throttle(function() {
              ...
              this.$inertia.replace(this.route('canvas.academiccategories.index', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
          },
        },
        methods: {
          destroy(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.delete(this.route('canvas.academiccategories.delete', id));
            }
          },
          restore(id) {
            if(confirm('tem a certeza?')) {
              this.$inertia.put(this.route('canvas.academiccategories.restore', id));
            }
          },
          setTrans(id, lang) {
            
              this.$inertia.get(this.route('canvas.academiccategories.getlang', {academiccategory: id, lang: lang}));
          },
          reset() {
    ...
    <div class="row">
      <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
          <h4 class="mb-0 font-size-18">Graus Académicos</h4>
          <div class="page-title-right">
            <b-button-group class="btn-group-sm mt-2">
                <b-button v-b-modal.action-info variant="secondary">
                    <i class="bx bx-info-circle"></i>
                </b-button>
                <inertia-link class="btn btn-secondary" :href="route('canvas.academiccategories.create')" method="get">
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered mb-0">
                ...
                <tbody>
                  <tr v-for="academiccategory in $page.props.academiccategories.data" :key="academiccategory.id">
                    <td>{{ academiccategory.name }}</td>
                    <td v-html="academiccategory.description"></td>
                    <td>
                      <b-button-group class="btn-group-sm mt-0">
                          <inertia-link class="btn btn-secondary" :href="route('canvas.academiccategories.edit', academiccategory.id)" method="get">
                              <i class="bx bx-pencil align-middle"></i>
                          </inertia-link>
                          <b-button variant="secondary" @click="destroy(academiccategory.id)">
                              <i class="bx bx-trash align-middle"></i>
                          </b-button>
                          <b-button variant="secondary" @click="restore(academiccategory.id)">
                          ...
                          <b-dropdown bottom size="sm">
                            ...
                            <b-dropdown-item class="font-size-sm" v-for="lang in $page.props.locales" :key="lang" @click="setTrans(academiccategory.id, lang)">
                            {{ lang.toUpperCase() }}
                            </b-dropdown-item>
                  ...
                  <tr v-if="$page.props.academiccategories.data.length === 0">
                    <td class="border-t px-6 py-4" colspan="5">No data found.</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <pagination :links="$page.props.academiccategories.links" :lastpage="$page.props.academiccategories.last_page" :firstpageurl="$page.props.academiccategories.first_page_url" :lastpageurl="$page.props.academiccategories.last_page_url" :nextpageurl="$page.props.academiccategories.next_page_url" />
          </div>
        </div>
      </div>
      <!-- end col -->
